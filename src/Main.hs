...
evalBlock (h:tl) =
  case h of
    ...
    -- Need to verify constness in typechecker
    ...
    ConstFor type_ ident expr1 expr2 stmt -> do
      ...
      loc <- newloc
      local (Map.insert ident loc) (evalFor start end loc stmt) >> evalBlock tl
    Ret expr -> Just <$> evalExpr expr
    VRet -> return (Just VVoid)
    Print expr -> evalExpr expr >>= (liftIO . putStrLn . toString) >> evalBlock tl
  ...
  where
    ...
    evalFor curInd end loc stmt = do
      env <- ask
      (mem, l) <- get
      if curInd <= end
        then writeMem (Map.insert loc (VNum curInd)) >> evalBlock [BStmt $ Block [stmt]] >>
             evalFor (curInd + 1) end loc stmt
        else writeMem id
...
main = do
  ...
  case args of
    [x] -> interpretFile x
    _ -> do
      putStrLn "Only one argument is expected"
      exitFailure
